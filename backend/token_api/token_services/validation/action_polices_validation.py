...
class ValidateActionWithActionPolicies:
    ...
    def __init__(self, action, device, action_polices):
        self.action = action
        self.device = device
        self.action_polices = action_polices if isinstance(action_polices, Iterable) and action_polices else [action_polices]
    ...
    def validate_device_against_policy(self, policy):
        ...
        allowed_devices = allowed_devices if isinstance(allowed_devices, Iterable) and allowed_devices else [allowed_devices]
        denied_devices = denied_devices if isinstance(denied_devices, Iterable) and denied_devices else [denied_devices]

        allowed = self.device in allowed_devices or not allowed_devices
    ...
    def validate_allowed_to_account_against_policy(self, policy):
        ...
        allowed_accounts = allowed_accounts if isinstance(allowed_accounts, Iterable) and allowed_accounts else [allowed_accounts]
        denied_accounts = denied_accounts if isinstance(denied_accounts, Iterable) and denied_accounts else [denied_accounts]

        allowed = self.action.to_account in allowed_accounts or not allowed_accounts
    ...
    def validate_allowed_from_account_against_policy(self, policy):
        ...
        allowed = self.action.from_account in allowed_accounts or not allowed_accounts
