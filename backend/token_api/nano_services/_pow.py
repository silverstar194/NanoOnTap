...
class POWService:
    ...
    def is_empty(cls):
        ...
        if ((len(temp_queue) > 0 and temp_queue[0][1] + 91 * 1000 <= int(round(time.time() * 1000))) or len(
                temp_queue) >= number_accounts() / 10):  ## 61 to prevent thread issues
            return False
        return True
    ...
    def POW_account_thread_asyc(cls, account, urgent=False):
        ...
        if not valid:
            try:
                ...
                logger.info(
                    'Generating PoW on start up address %s frontier %s urgent %s' % (account.address, frontier, urgent))
            except Exception as e:
